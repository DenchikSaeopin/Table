  ...
  <div>
    <v-toolbar flat dense>
      ...
      <v-btn-toggle> 
        ...
        <v-tooltip bottom> 
          <template v-slot:activator="{ on, attrs }">  
            <v-btn  @click="editItem" icon x-small fab v-bind="attrs" v-on="on" :disabled="selected.length != 1">
        ...
        <v-tooltip bottom> 
          <template v-slot:activator="{ on, attrs }">  
            <v-btn  @click="dialog = true" icon x-small fab v-bind="attrs" v-on="on"><v-icon>mdi-table-row-plus-after</v-icon></v-btn>
          </template>  
    ...
    <v-data-table
      ...
      class="elevation-1"
    >    
      <template v-for="slot in field_slots" v-slot:[slot.slot_name] = {item}>{{ slot.func(item[slot.field], slot.DisplayFormat, slot.PropType) }}</template>
      ...
      for (let value in this.dataset[0]) {
        if (!(this.hiddenColumns.find(item => item.name == value))) {
          this.ViewProperties.forEach ((item) => {
            for(let key in item) {
              if(key == value) {
                result.push({text: item[key].Name, value: value, align: item[key].Align, sortable: item[key].Sortable,  }) 
              }
            }        
          })                                             
        }
